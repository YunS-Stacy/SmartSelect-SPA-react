  ...
  return new Promise(function (resolve, reject) {
    ...
    fetch(`${endpoint}${params}`)
        ...
        .then(res => {
          parseString(res, (err, result) => {
            if (err) return reject(err);
            if (!result["Comps:comps"].response) return reject(new Error('No response from GetDeepComps'));
 ...
 */
Zillow.getCompsFromZpid = async function (zpid) {
  try {
    ...
    const zestimateObj = await Zillow.getComps(zpid);
